...
3. アドレス設定プロセス 以下の手順を、バス上の各センサに対して順番に実行します。
  * まず、1台目のセンサの Xshut ピンをHIGHにし、パワーオン状態にします。この時点では、このセンサだけがデフォルトアドレス 0x52 で応答します。
  * 次に、ホストは VL53LX_SetDeviceAddress() 関数を呼び出し、このセンサに新しい一意のI2Cアドレス（例: 0x54）を割り当てます。
